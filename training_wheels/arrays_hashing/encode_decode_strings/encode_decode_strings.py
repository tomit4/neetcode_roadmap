...
class Solution:
    # Establish an empty string res
    # iterate over the strs array
    # append res the length of the string plus a delimeter plus the string itself
    # return the res
    def encode(self, strs: List[str]) -> str:
        return ""

    # establish a res array and a pointer named i
    # while loop from i to the length of the s
    # inside while loop, establish second pointer j equal to i
    # second while loop, while s at j index does not equal delimeter
    # keep incrementing j until delimter is found...
    # after inner while loop is done, assign new variable length to s slice from i to j, cast to int
    # then append to res the s slice from j + 1 to  j + 1 + length
    # reassign i to j + 1 + length
    # return res
